...
output "consumption_budget_subscriptions_filter" {
  ...
  value       = { for k, v in azurerm_consumption_budget_subscription.consumption_budget_subscriptions : k => one(v.filter) if v.filter != null && length(v.filter) > 0 }
}
...
output "consumption_budget_subscriptions_time_period" {
  ...
  value       = { for k, v in azurerm_consumption_budget_subscription.consumption_budget_subscriptions : k => one(v.time_period) if v.time_period != null && length(v.time_period) > 0 }
}
